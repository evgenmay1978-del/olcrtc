...
import ru.maestrovpn.app.data.TUN2SOCKS_CONFIG_FILE_NAME
...
import ru.maestrovpn.app.data.repository.LocationsRepository
...
import ru.maestrovpn.app.vpn.UpstreamCandidate
import ru.maestrovpn.app.vpn.UpstreamNetworkSelector
import ru.maestrovpn.app.vpn.UpstreamTransport
...
class MaestroVpnVpnService : VpnService() {
    ...
    private val networkCallback = object : ConnectivityManager.NetworkCallback() {
        ...
        override fun onLost(network: Network) {
            ...
            networkLossJob = scope.launch {
                ...
                if (MaestroVpnVpnState.status.value is VpnStatus.Connected ||
                    MaestroVpnVpnState.status.value is VpnStatus.Reconnecting
                ) {
        ...
        private fun handleNetworkChange(network: Network, reason: String) {
            ...
            if (currentNetwork == upstream) {
                if (MaestroVpnVpnState.status.value is VpnStatus.Reconnecting &&
            ...
            when (MaestroVpnVpnState.status.value) {
    ...
    override fun onCreate() {
        ...
        wakeLock = (getSystemService(Context.POWER_SERVICE) as PowerManager)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "MaestroVpn::VpnWakeLock")
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            MaestroVpnVpnActions.ACTION_STOP_VPN -> {
                addLog("Stop VPN requested")
                cleanup()
                return START_NOT_STICKY
            }

            MaestroVpnVpnActions.ACTION_START_VPN -> Unit
            else -> {
                cleanup()
                stopSelf()
                return START_NOT_STICKY
    ...
    private fun installMobileCallbacks() {
        Mobile.setProtector(object : SocketProtector {
            override fun protect(fd: Long): Boolean {
                ...
                return this@MaestroVpnVpnService.protect(fd.toInt())
            }
        })
        Mobile.setProviders()
    ...
    private fun loadStartOptions(intent: Intent): StartOptions {
        ...
        val socksPort = if (intent.hasExtra(MaestroVpnVpnActions.EXTRA_SOCKS_PORT)) {
            intent.getIntExtra(
...
//            }
    ...
    private fun startTun2socks(pfd: ParcelFileDescriptor): Boolean {
        return try {
            ...
            tun2socksThread = thread(name = "MaestroVpnTun2Socks", isDaemon = true) {
                try {
                    val result = startTun2socksNative(configFile.absolutePath, nativeFd)
                    if (MaestroVpnVpnState.status.value !is VpnStatus.Stopping && result != 0) {
    ...
    private fun applySplitTunneling(builder: Builder): Boolean {
        return when (splitTunnelMode) {
            AndroidSplitTunnelMode.AllApps -> {
                addDisallowedApp(builder, packageName, "MaestroVpn")
    ...
    private fun startWatchdog() {
        ...
        watchdogJob = scope.launch {
            while (isActive && MaestroVpnVpnState.status.value is VpnStatus.Connected) {
    ...
    private fun cleanup(stopService: Boolean = true) {
        ...
        val status = MaestroVpnVpnState.status.value
    ...
    private fun noteRtcFailure(
        ...
        if (MaestroVpnVpnState.status.value !is VpnStatus.Connected) return
    ...
    private fun requestTransportRecovery(
        ...
        recoveryJob = scope.launch {
            ...
            val currentStatus = MaestroVpnVpnState.status.value
    ...
    private fun scheduleTransportRetry(
        ...
        recoveryJob = scope.launch {
            ...
            if (MaestroVpnVpnState.status.value !is VpnStatus.Reconnecting) return@launch
    ...
    private fun shouldRestartForStartCommand(): Boolean {
        return when (MaestroVpnVpnState.status.value) {
            VpnStatus.Connected,
        ...
        NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID)
            ...
            .addAction(
                ...
                PendingIntent.getService(
                    this,
                    0,
                    Intent(this, MaestroVpnVpnService::class.java).apply { action = ACTION_STOP_VPN },
    ...
    private fun setStatus(status: VpnStatus) {
        MaestroVpnVpnState.setStatus(status)
    }
    ...
    private class AuthenticatedSocksProxy(
        ...
        fun start() {
            ...
            serverSocket = server
            acceptThread = thread(name = "MaestroVpnSocksProxy", isDaemon = true) {
                acceptLoop(server)
            }
        ...
        private fun acceptLoop(server: ServerSocket) {
            while (!stopped) {
                ...
                thread(name = "MaestroVpnSocksProxyClient", isDaemon = true) {
                    try {
                        handleClient(client)
                    } finally {
        ...
        private fun relay(from: Socket, to: Socket, name: String): Thread {
            return thread(name = "MaestroVpnSocksRelay-$name", isDaemon = true) {
                runCatching {
    ...
    companion object {
            ...
            return synchronized(nativeLibrariesLock) {
                if (nativeLibrariesLoaded) {
                    ...
                    try {
                        ...
                        System.loadLibrary("maestrovpn_tun2socks")
        ...
        const val ACTION_START_VPN = MaestroVpnVpnActions.ACTION_START_VPN
        const val ACTION_STOP_VPN = MaestroVpnVpnActions.ACTION_STOP_VPN
        ...
        private const val NOTIFICATION_CHANNEL_ID = "maestrovpn_vpn"
        private const val NOTIFICATION_ID = 100
        private const val TAG = "MaestroVpnVpnService"

        private fun addLog(msg: String) {
            MaestroVpnVpnState.addLog(msg)
        }
    }
}
