package ru.maestrovpn.app.desktop
...
internal object DesktopPaths {
    ...
    fun appDataDir(): Path {
        ...
        val dir = when (os) {
            DesktopOs.MacOS -> home.resolve("Library").resolve("Application Support").resolve("MaestroVpn")
            DesktopOs.Windows -> {
                val appData = System.getenv("APPDATA")?.takeIf { it.isNotBlank() }
                (appData?.let { Path(it) } ?: home.resolve("AppData").resolve("Roaming")).resolve("MaestroVpn")
            }
            DesktopOs.Linux,
            DesktopOs.Other -> home.resolve(".maestrovpn")
        }
