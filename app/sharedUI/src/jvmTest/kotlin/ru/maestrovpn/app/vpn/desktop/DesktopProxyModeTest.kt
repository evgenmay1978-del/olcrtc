package ru.maestrovpn.app.vpn.desktop

import ru.maestrovpn.app.data.model.LocationConfig
import ru.maestrovpn.app.vpn.olcRtcNativeLibrarySpec
...
class DesktopProxyModeTest {
    ...
    fun linuxTunConfigCanRunRouteScriptsInsidePrivilegedTunnelProcess() {
        val config = LinuxTunController.configContent(
            socksPort = 10810,
            postUpScript = "/tmp/maestrovpn-up.sh",
            preDownScript = "/tmp/maestrovpn-down.sh"
        )

        assertContains(config, "port: 10810")
        assertContains(config, "post-up-script: /tmp/maestrovpn-up.sh")
        assertContains(config, "pre-down-script: /tmp/maestrovpn-down.sh")
    }
    ...
    fun linuxTunConfigUsesLocalSocksAndIpv4MapDns() {
        ...
        assertContains(config, "name: maestrovpn0")
    ...
    fun windowsTunCommandUsesTun2SocksWintunAndLocalSocks() {
        val command = WindowsTunController.tun2SocksCommand(
            tun2SocksBinary = Path.of("C:/MaestroVpn/bin/tun2socks-windows-amd64.exe"),
            socksPort = 10812
        )

        assertContains(command, "C:/MaestroVpn/bin/tun2socks-windows-amd64.exe")
        assertContains(command, "--device")
        assertContains(command, "MaestroVpn")
    ...
    fun windowsTunAdministratorRestartUsesRunAsAndPreservesArguments() {
        val script = WindowsTunController.restartAsAdministratorScript(
            command = "C:/Olc's/MaestroVpn.exe",
            arguments = listOf("--flag", "C:/Path With Space/data"),
            workingDirectory = "C:/MaestroVpn Data"
        )

        assertContains(script, "FilePath = 'C:/Olc''s/MaestroVpn.exe'")
        assertContains(script, "Verb = 'RunAs'")
        assertContains(script, "ArgumentList = '--flag \"C:/Path With Space/data\"'")
        assertContains(script, "WorkingDirectory = 'C:/MaestroVpn Data'")
    ...
    fun linuxTunScriptsRouteUserTrafficThroughTunAndKeepRootDirect() {
        ...
        assertContains(up, "ip route add default dev maestrovpn0 table 51820")
        assertContains(up, "ip rule add lookup 51820 pref 20")
        assertContains(up, "resolvectl dns maestrovpn0 1.1.1.1")
        assertContains(down, "ip rule del uidrange 0-0 lookup main pref 10")
        assertContains(down, "ip route flush table 51820")
        assertContains(down, "resolvectl revert maestrovpn0")
    }
}
