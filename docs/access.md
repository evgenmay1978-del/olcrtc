...
Сервер считает по каждому клиенту: число потоков и переданные байты
(in/out). Во встраиваемом API (`pkg/olcrtc/tunnel`) эти данные доступны через
хук `OnTraffic` (вызывается на каждый завершённый поток) и снимок
`Server.Usage()`. Это основа для биллинга по объёму.
