...
func createAgentBase(config *AgentConfig) (*Agent, error) {
	...
	agent := &Agent{
		...
		// olcRTC fork: always skip TURN/DTLS relay certificate verification.
		// olcRTC rides on third-party whitelisted SFUs (Jitsi) whose TURN relays
		// occasionally serve expired/mismatched TLS certs; the tunnel payload is
		// already end-to-end encrypted (XChaCha20-Poly1305), so verifying the
		// relay cert adds no security but breaks connectivity, especially on
		// mobile/CGNAT where the relay is mandatory. See third_party/README.md.
